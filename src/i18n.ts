...
const en = {
  ...
  graph: {
    heading: 'Distribution Insight',
    subtitle: (count: number, span: string) => `${count} indexed points, ${span} shown in degrees.`,
    arcSpan: 'arc span',
    angleSpan: 'angle span',
    empty: 'No valid placements',
    aria: 'Distribution insight graph',
...
const ja: UiText = {
  ...
  graph: {
    heading: '分布インサイト',
    subtitle: (count: number, span: string) => `${count}個のインデックス点、${span}を度数で表示しています。`,
    arcSpan: '円弧スパン',
    angleSpan: '角度スパン',
    empty: '有効な配置がありません',
    aria: '分布インサイトグラフ',
